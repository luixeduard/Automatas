...
T_estado tabla_aux2[500][100];//Tabla auxiliar
...
int buscar_operador(char exp[]){
 ...
 void thompson(char exp[],T_estado tabla[500][100]){//La tabla  thompson se llena aquíLa tabla  thompson se llena aquíLa tabla  thompson se llena aquíLa tabla  thompson se llena aquí
	...
	if (aux==false){
		switch (exp[operador]){
			case '*':
				tabla[1][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[1][char_index(let,'?')+1].trans_estado[1]=4;
				tabla[2][char_index(let,exp[operador-1])+1].trans_estado[0]=3;
				tabla[3][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[3][char_index(let,'?')+1].trans_estado[1]=4;
				ultimo_estado=4;
			break;
			case '|':
				tabla[1][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[1][char_index(let,'?')+1].trans_estado[1]=4;
				tabla[2][char_index(let,exp[operador-2])+1].trans_estado[0]=3;
				tabla[3][char_index(let,'?')+1].trans_estado[0]=6;
				tabla[4][char_index(let,exp[operador-1])+1].trans_estado[0]=5;
				tabla[5][char_index(let,'?')+1].trans_estado[0]=6;
				ultimo_estado=6;
			break;
			case '.':
				tabla[1][char_index(let,exp[operador-2])+1].trans_estado[0]=2;
				tabla[2][char_index(let,'?')+1].trans_estado[0]=3;
				tabla[3][char_index(let,exp[operador-1])+1].trans_estado[0]=4;
				ultimo_estado=4;
			break;
		}
		aux=true;
...
//Segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte segúnda parte
		...
		switch (exp[operador]){
			case '*':
				printf("Ultimo_estado: %i\n",ultimo_estado);
				nuevo_estado_tabla(1,tabla);
				tabla[1][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[1][char_index(let,'?')+1].trans_estado[1]=ultimo_estado+1;
				tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[1]=ultimo_estado+1;
				printf("Ultimo_estado: %i\n",ultimo_estado);
				ultimo_estado=ultimo_estado+1;
			break;
			case '.':
				if (exp[operador-1]!='Ç'){
					tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[0]=ultimo_estado+1;
					tabla[ultimo_estado+1][char_index(let,exp[operador-1])+1].trans_estado[0]=ultimo_estado+2;
					ultimo_estado=ultimo_estado+2;
				}
				if (exp[operador-2]!='Ç'){
					tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[0]=ultimo_estado+1;
					tabla[ultimo_estado+1][char_index(let,exp[operador-2])+1].trans_estado[0]=ultimo_estado+2;
					ultimo_estado=ultimo_estado+2;
				}
			break;
			case '|':
				/*nuevo_estado_tabla(2,tabla);
				tabla[1][char_index(let,'?')+1].trans_estado[0]=2;
				tabla[1][char_index(let,'?')+1].trans_estado[1]=4;
				tabla[2][char_index(let,exp[operador-1])+1].trans_estado[0]=ultimo_estado-1;
				tabla[ultimo_estado-1][char_index(let,'?')+1].trans_estado[0]=ultimo_estado;
				tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[0]=ultimo_estado+1;
				ultimo_estado=ultimo_estado+1;*/
				if(exp[operador-1]!='Ç'){
					nuevo_estado_tabla(2,tabla);
				...
				if(exp[operador-2]!='Ç'){
					...
					tabla[1][char_index(let,'?')+1].trans_estado[0]=2;
					tabla[1][char_index(let,'?')+1].trans_estado[1]=4;
					tabla[2][char_index(let,exp[operador-2])+1].trans_estado[0]=ultimo_estado-1;
					tabla[ultimo_estado-1][char_index(let,'?')+1].trans_estado[0]=ultimo_estado;
					tabla[ultimo_estado][char_index(let,'?')+1].trans_estado[0]=ultimo_estado+1;
					ultimo_estado=ultimo_estado+1;
				}
	...
	while (a[j]!=0){;
		...
		if(a[j+1]!=0){
			...
			if(strstr(aux,",")!=0){
				int cc=cont;
				for(int k=0;k<=cc;k++){
					if(strcmp(esnew[k],aux)==0){
						nel+=1;
					}
					
				}
				if(nel==0){
					strcpy(esnew[cont],aux);
					cont+=1;
				}
...
void est_nuevo(int long_alfabeto){
	...
	while (i<=est+contesta){
		...
		while (j<=long_alfabeto){
			if (tabla[i][j].trans_estado[0]!=0){
				new_estate(tabla[i][j].trans_estado);
			}
			else{
			}
			j=j+1;
...
void renombrar_estados(){
	int i,j;
	i=1;
	while(i<=est){
		j=1;
		while(j<strlen(let)){
			if(int_index(tabla[i][j].trans_estado,-1)>=2){
				int_arari_clean(tabla[i][j].trans_estado);
				tabla[i][j].trans_estado[0]=ultimo_estado+1;
			}
			j=j+1;
		}
		i=i+1;
...
//Termina la ejecucion de la tabla de thompsonTermina la ejecucion de la tabla de thompsonTermina la ejecucion de la tabla de thompsonTermina la ejecucion de la tabla de thompson
	...
	//ver_tabla_total(tabla);
	printf("FIN TABLA THOMSON");
	for(int i=0;i<=est;i++){
		estados_acep(longitud_alfabeto);
	}
	union_de_lambdas();//.................................................................................................................. Tenga cuidado
	exec=1;
	//renombrar_estados();//.................................................................................................................. Tenga cuidado x2
	ver_tabla(longitud_alfabeto,tabla);
	est_nuevo(longitud_alfabeto);
	for(int w=0;w<=500;w++){
		if(strlen(esnew[w])>0){
			contesta+=1;
		}
	...
	int parar;
	//Ahora debemos iterar los nuevos estados//
	while(parar!=contesta){
	est_nuevo(longitud_alfabeto);
	for(int w=contesta;w<=500;w++){
		if(strlen(esnew[contesta])==0){
			parar=contesta;
		}
		else if(strlen(esnew[w])>0){
			contesta+=1;
		}
	}
	}
